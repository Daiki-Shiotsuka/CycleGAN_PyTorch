...
def train_loop(opts):
    ...
    train_dataloader = DataLoader(CycleGANDataset(opts.dataset_name, transform), batch_size=opts.batch_size, shuffle=True, num_workers=opts.n_cpu)
    ...
    for epoch in range(opts.start_epoch, end_epoch):
        for index, batch in enumerate(train_dataloader):
            ...
            print(f"\r[Epoch {epoch+1}/{opts.epochs-opts.start_epoch}] [Index {index}/{len(train_dataloader)}] [D_A loss: {loss_d_a.item():.4f}] [D_B loss: {loss_d_b.item():.4f}] [G loss: adv: {loss_gan.item():.4f}, cycle: {loss_cycle.item():.4f}, identity: {loss_identity.item():.4f}]")
...
def create_parser():
    ...
    # トレーニング用ハイパーパラメータ
    parser.add_argument('--dataset_name', type=str, default='day2night', help='使用するデータセット.')
    ...
    #test
    parser.add_argument('--test_epoch', type=str, default='latest', help='テストに使うepoch')
